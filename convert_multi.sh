...
LOAD="${1:-../models/qwen3-8b-hybrid-vit-stage0}"
SAVE="${2:-../models/qwen3-8b-hybrid-vit-stage0_mcore_tp1_pp1}"
TP="${3:-1}"
PP="${4:-1}"

SAVE_LANGUAGE_MODEL=./tmp/language-mcore
SAVE_VISION_MODEL=./tmp/vision-model-mcore
SAVE_SIGLIP2=./tmp/vision-model-siglip2
SAVE_DINOV3=./tmp/vision-model-dinov3
...
# merge
python $CONVERT_CHECKPOINT_PATH/custom/innovator_vl/merge_megatron_hybrid.py \
    --megatron_path $AIAK_MAGATRON_PATH \
    --language_model_path $SAVE_LANGUAGE_MODEL/release \
    --ricevit_path $SAVE_VISION_MODEL/release \
    --siglip2_path $SAVE_SIGLIP2/release \
    --dinov3_path $SAVE_DINOV3/release \
